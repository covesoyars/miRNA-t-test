...
# represents an miRNA. It finds, using a t-test, miRNAs that are differentially expressed between the cancers.
import csv
...
def read_headers(expressionfile):
    ...
    with open(expressionfile) as file:  # open file
        headers = next(file).strip().split(
            ',')  # headers = first row of file, stripped of formatting characters and split
        # by commas
    return headers
...
def get_differentially_expressed(expression, names, type1, type2):
    ...
    for i in range(0, len(expression)):
        ...
        type1_mirnas = [miRNA[ind] for ind in type1]
        type2_mirnas = [miRNA[ind] for ind in type2]

        pval = stats.ttest_ind(type1_mirnas, type2_mirnas).pvalue  # calculate p value
        print(pval)
        if pval <= 0.05 / len(
                expression):  # if p value passes test with Bonferroni correction, (see comments below function)
...
ID = miRNA_ID('miRNA_rownames.csv')  # read in miRNA IDs

header = read_headers("miRNA_project_data.csv")  # read in header
